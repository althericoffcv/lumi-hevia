...
export class Button {
  ...
  async run(jid, conn, quoted = '', { bypass = false, ...options } = {}) {
    if (this._type === 0) {
      const headerData = this._data
        ? await prepareWAMessageMedia(this._data, { upload: conn.waUploadToServer })
        : {}
      
      const thumbnailData = this._thumbnail && !this._data
        ? await prepareWAMessageMedia({ image: { url: this._thumbnail } }, { upload: conn.waUploadToServer })
        : {}
      ...
      const message = {
        ...
        header: {
          ...
          hasMediaAttachment: !!(this._data || this._thumbnail),
          ...headerData,
          ...thumbnailData,
        },
      }
